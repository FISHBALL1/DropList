...
//  DropList
//
//  Created by 牛尧 on 16/7/24.
//  Copyright © 2016年 FISHBALL1. All rights reserved.
//

import UIKit

public protocol DropListDelegate:class{
    func dropDownMenu(menu:DropList!, didInput text:String!)
    func dropDownMenu(menu:DropList!, didChoose index:Int)
}

@IBDesignable public class DropList: UIView , UITableViewDataSource ,UITableViewDelegate,UITextFieldDelegate{
    
    public weak var delegate:DropListDelegate?
    
    public var inputClosure: ((DropList , text: String) ->Void )?
    
    public var chooseClosure: ((DropList , index: Int) ->Void )?
    ...
    public lazy var optionsList:UITableView = { //下拉列表
        ...
        //let table = UITableView(frame: CGRectMake(self.frame.origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width, 0), style: .Grouped)//设置成组就可以上下滑动了
